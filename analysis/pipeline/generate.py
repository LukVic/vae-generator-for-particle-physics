...
def main():
    APPROACH = 'std_h' # 'std', 'sym', 'std_h', 'sym_h' 
    TRAIN = True
        ...
        if TRAIN:
            ...
            for epoch in range(gen_params["num_epochs"]):
                ...
                if elbo_min > loss.item():
                    ...
                    elbo_min = loss.item()
            data_gen(PATH_DATA, DATA_FILE, PATH_MODEL = f'{PATH_MODEL}{directory}{DATA_FILE}.pth', PATH_JSON=f'{PATH_JSON}hyperparams.json', TYPE=APPROACH, scaler=scaler, reaction=classes[REACTION])
        else:
