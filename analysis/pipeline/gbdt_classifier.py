...
def feature_importnace_gbdt(model, X_train, params):
    PATH_FEATURES = f'../features/most_important_gbdt_{params["features"]["features_num"]}_{params["generate"]["general"]["reaction"]}' # feature directory path
    
    
    plt.clf()
    importances = model.named_steps['clf'].feature_importances_

    indices = np.argsort(importances)[::-1][:params["features"]["features_num"]]  
    #print(indices)
    #plt.figure(figsize=(10, 6))
    plt.title(f'Top {params["features"]["features_num"]} Feature Importances')
    plt.bar(range(params["features"]["features_num"]), importances[indices], align="center")
    plt.xticks(range(params["features"]["features_num"]), X_train.columns[indices], rotation=90)
    plt.xlim([-1, params["features"]["features_num"]])
    plt.tight_layout()
