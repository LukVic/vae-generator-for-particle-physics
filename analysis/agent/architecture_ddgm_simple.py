...
class DDGM(nn.Module):
    ...
    def forward(self, x):
        ...
        qs, zs = self.forward_diff(x, self.dif_depth, self.beta)
        ...
        for idx in range(len(self.encoders) - 1, 0, -1):
            mu, log_sigma = self.encoders[idx](zs[idx+1])
            p = torch.distributions.Normal(mu,torch.exp(log_sigma))
            ps.append(p)
        ...
        pz = torch.distributions.Normal(torch.zeros_like(x), torch.ones_like(x))
        ...
        KLD_REST = 0
        for idx in range(1, len(qs) -2):
            KLD_REST += torch.distributions.kl_divergence(qs[idx], ps[idx]).sum(dim=1)
        
        KLD_ALL = KLD_FIRST + KLD_REST + KLD_LAST + BCE_B
    ...
    def forward_diff(self, x, depth, beta):
        distrs, samples = [], [x]
        xt = x
        
        for t in range(depth):
            
            q = torch.distributions.Normal(np.sqrt(1 - beta)*xt, beta*torch.ones_like(xt))
            xt = q.sample()
            distrs.append(q)
            samples.append(xt)
