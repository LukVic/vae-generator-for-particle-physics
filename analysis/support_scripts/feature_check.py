...
def feature_check(path):
    ...
    df_generated = pd.read_csv(f'{path}data/bkg_all_input/generated_df_bkg_all_pres_strict_E100_S27611_std.csv')
    df_generated_sym = pd.read_csv(f'{path}data/bkg_all_input/generated_df_bkg_all_pres_strict_E100_S27611_std.csv')
    ...
    for feature in feature_list:
        ...
        #     print(max_all)
        #     exit()
        h_feature_original = ROOT.TH1F(f"h_{feature[0]}_original",f";{feature[0]}; events (normalized)",500, min_all, max_all)
        h_feature_generated_std = ROOT.TH1F(f"h_{feature[0]}_generated",f";{feature[0]}; events (normalized)",500, min_all, max_all)
        h_feature_generated_sym = ROOT.TH1F(f"h_{feature[0]}_generated_sym",f";{feature[0]}; events (normalized)",500, min_all, max_all)
